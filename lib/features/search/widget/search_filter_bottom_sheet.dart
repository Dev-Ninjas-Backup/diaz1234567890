...
class SearchFilterBottomSheet extends StatelessWidget {
  const SearchFilterBottomSheet({super.key, required this.searchControllerTag});
    ...
    return Obx(
      () => Column(
        children: [
          RangeSlider(
            values: RangeValues(
              controller.priceStart.value,
              controller.priceEnd.value,
            ),
            min: 0,
            max: 5000000,
          ...
          Row(
            ...
            children: [
              Text(
                '\$${controller.priceStart.value.toInt()}',
                style: const TextStyle(fontSize: 12, color: Color(0xFF4A4D4D)),
              ),
              Text(
                '\$${controller.priceEnd.value.toInt()}',
                style: const TextStyle(fontSize: 12, color: Color(0xFF4A4D4D)),
              ),
            ],
          ),
  ...
  Widget _rangeInputs({
    ...
    return Row(
      children: [
        Expanded(
          child: _numberField(controller: leftController, hint: leftHint),
        ),
        const SizedBox(width: 10),
        Expanded(
          child: _numberField(controller: rightController, hint: rightHint),
        ),
      ],
    );
  }
